	...
	if($called_position == 'after_module_proc' && $this->act == "procBoardInsertDocument") {
		...
		if($this->module_info->use_anonymous == 'Y') {
				...
				if(isset($logged_info)) {
					$args->nick_name = "익명_" . (hexdec(substr(md5($args->document_srl.$addon_info->security_code.$logged_info->email_address),0,6)) % 100);
				} else {
					$args->nick_name = "익명_" . (hexdec(substr(md5($args->document_srl.$addon_info->security_code.$_SERVER['REMOTE_ADDR']),0,6)) % 100);
				}
